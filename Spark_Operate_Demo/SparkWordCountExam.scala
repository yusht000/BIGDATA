...
object SparkWordCountExam {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 利用 spark 中 封装方法 进行 分组聚合
    val wordToTotalRDD: RDD[(String, Int)] = wordToOneRDD.reduceByKey((x, y)=>{x + y})

    val resultArr: Array[(String, Int)] = wordToTotalRDD.collect()
